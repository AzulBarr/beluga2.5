...
def main():
    ...
    try:
        pkg_share = get_package_share_directory('belugaslam_example')
    except Exception as e:
        print(f"Error: Could not find package 'belugaslam_example'. Did you source the workspace?")
        sys.exit(1)

    launch_file = os.path.join(pkg_share, 'example', 'launch', 'beluga_rosbag_belugaslam.xml')
